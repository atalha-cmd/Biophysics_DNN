...
def model(p, L, k):
    ...
    X_test_images_raw = X_test_combined[:, :n_img_feats].reshape(-1, 200, 12, 1)
    X_test_ef_raw = X_test_combined[:, n_img_feats:]
    ...
    for train_index, val_index in kf.split(X_train_combined):
        ...
        X_train_centered = datagen.standardize(X_train_images_fold.copy())
        X_val_centered = datagen.standardize(X_val_images.copy())
        X_test_centered_fold = datagen.standardize(X_test_images_raw.copy())  
        ...
        print(X_test_centered_fold.shape, X_test_centered_fold.mean(), X_test_centered_fold.std())
        ...
        X_test_centered_fold = X_test_centered_fold.reshape((X_test_centered_fold.shape[0], X_test_centered_fold.shape[1], X_test_centered_fold.shape[2]))
        ...
        X_test_ef_scaled_fold = scaler_X_fold.transform(X_test_ef_raw)
        ...
        y_test_scaled_fold = scaler_y_fold.transform(np.array(y_test).reshape(-1, 1)).flatten()
        ...
        y_ef = Dense(128, activation='relu', kernel_initializer='he_uniform', kernel_regularizer=l2(0.01))(inputEF)
        y_ef = Dropout(0.15)(y_ef)
        y_ef = BatchNormalization()(y_ef)
        y_ef = Dense(64, activation='relu', kernel_regularizer=l2(0.02))(y_ef)
        modelEF = keras.Model(inputs=inputEF, outputs=y_ef)
        ...
        mse_scaled = mean_squared_error(y_val_scaled, y_val_pred_scaled.ravel())
        ...
        if mse_scaled < best_mse:
            best_mse = mse_scaled
            best_base_name = f"best_model_p{p}_L{L}_fold_{counter}"

            os.makedirs("models", exist_ok=True)
            model_filename = os.path.join("models", f"{best_base_name}.keras")
            model_merged.save(model_filename)

            joblib.dump(scaler_X_fold, os.path.join("models", f"{best_base_name}_scaler_X.pkl"))
            joblib.dump(scaler_y_fold, os.path.join("models", f"{best_base_name}_scaler_y.pkl"))

            np.save(os.path.join("models", f"{best_base_name}_img_mean.npy"), datagen.mean)
            np.save(os.path.join("models", f"{best_base_name}_img_std.npy"), datagen.std)
    ...
    scaler_X_best = joblib.load(os.path.join("models", f"{best_base_name}_scaler_X.pkl"))
    scaler_y_best = joblib.load(os.path.join("models", f"{best_base_name}_scaler_y.pkl"))
    img_mean = np.load(os.path.join("models", f"{best_base_name}_img_mean.npy"))
    img_std = np.load(os.path.join("models", f"{best_base_name}_img_std.npy"))


    X_test_images_best = X_test_images_raw.copy()
    X_test_images_best = (X_test_images_best - img_mean) / (img_std + 1e-7)
    X_test_centered = X_test_images_best.reshape((X_test_images_best.shape[0], X_test_images_best.shape[1], X_test_images_best.shape[2]))

    X_test_ef_scaled = scaler_X_best.transform(X_test_ef_raw)
    y_test_scaled = scaler_y_best.transform(np.array(y_test).reshape(-1, 1)).flatten()
...
if __name__ == "__main__":
    ...
    os.makedirs("evals/both", exist_ok=True)
    df.to_csv(f"evals/both/evaluation_metrics_both_p{p}_L{L}_CE.txt", sep='\t', index=False)
